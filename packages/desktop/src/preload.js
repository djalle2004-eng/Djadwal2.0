...
contextBridge.exposeInMainWorld('db', {
  ...
  addProfessor: (name, email, metadata) => ipcRenderer.invoke('add-professor', name, email, metadata),
  updateProfessor: (id, name, email, metadata) => ipcRenderer.invoke('update-professor', id, name, email, metadata),
  ...
  updateExtraSession: (id, session) => ipcRenderer.invoke('update-extra-session', id, session),
  ...
  // دوال فترات الزمن
  getTimeSlots: () => ipcRenderer.invoke('get-time-slots'),
  ...
  saveUserPermissions: (userId, permissionsJson) => ipcRenderer.invoke('save-user-permissions', userId, permissionsJson),
  ...
  exportToJSON: (academicYearId) => ipcRenderer.invoke('export-to-json', academicYearId),
  importFromJSON: (jsonData, mode) => ipcRenderer.invoke('import-from-json', jsonData, mode),
  ...
  scheduleAutoBackup: (settings) => ipcRenderer.invoke('schedule-auto-backup', settings),
...
// ---> START: Exposer l'écouteur IPC <--- 
contextBridge.exposeInMainWorld('electronAPI', {
  // Fonction pour enregistrer un écouteur
  onTriggerSearch: (callback) => {
    ipcRenderer.on('trigger-search', (event, ...args) => callback(event, ...args));
  },
  // Fonction pour supprimer l'écouteur (important pour éviter les fuites de mémoire)
  removeTriggerSearchListener: (callback) => {
    ipcRenderer.removeListener('trigger-search', callback);
  },
  // Optionnel: Envoyer un message au processus principal quand le rendu est prêt
  rendererReady: () => ipcRenderer.send('renderer-ready-for-search')
});

// Exposer electron pour استدعاءات IPC مباشرة
contextBridge.exposeInMainWorld('electron', {
  invoke: (channel, ...args) => ipcRenderer.invoke(channel, ...args)
});
// ---> END: Exposer l'écouteur IPC <---