...
/**
 * Re-run Migration for Failed Tables
 * إعادة تشغيل النقل للجداول الفاشلة
 */
...
async function retryMigration() {
  ...
  console.log('🔄 إعادة تشغيل النقل للجداول الفاشلة...');
  ...
  try {
    ...
    // إعادة نقل الجداول التي فشلت سابقاً
    console.log('🎯 إعادة نقل الجداول الفاشلة...');
    
    // 1. جدول professors (فشل بسبب أعمدة مفقودة)
    await migrateTable('professors', sqliteDb, neonClient, { skipDuplicates: true });
    
    // 2. جدول users (فشل بسبب عمود مفقود)
    await migrateTable('users', sqliteDb, neonClient, { skipDuplicates: true });
    
    // 3. إعادة نقل assignments للبيانات الفاشلة
    console.log('\n🔄 إعادة نقل assignments للبيانات الفاشلة...');
    ...
    if (totalErrors > 0) {
      ...
      console.log('📄 تفاصيل الأخطاء في retry-migration-errors.json');
      
      const fs = require('fs');
      fs.writeFileSync(
        'retry-migration-errors.json',
        JSON.stringify(stats.errors, null, 2)
      );
    }
    
    console.log('\n✅ تم إعادة النقل بنجاح!');
