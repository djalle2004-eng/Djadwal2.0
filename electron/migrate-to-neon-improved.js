...
/**
 * Migration Script: SQLiteCloud → Neon PostgreSQL (Improved)
 * نقل البيانات من SQLiteCloud إلى Neon مع معالجة الأخطاء
 */
...
async function migrateTable(tableName, sqliteDb, neonClient, options = {}) {
  ...
  try {
    ...
    for (const row of rows) {
      try {
        // تنظيف أسماء الأعمدة (إزالة المسافات والرموز الخاصة)
        const cleanedRow = {};
...
async function migrate() {
  ...
  try {
    ...
    await migrateTable('specializations', sqliteDb, neonClient); // سيتم تخطيه إذا لم يكن موجوداً
